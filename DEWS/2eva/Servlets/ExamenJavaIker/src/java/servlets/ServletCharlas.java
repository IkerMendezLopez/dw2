...
public class ServletCharlas extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("grabar")==null){ 
            ...
            double tarifa = 70;
            ArrayList<Sala> lstSala = bd.salasPreparadas();
            String hora = request.getParameter("horaValor");
            String tema = request.getParameter("tema");
            String[] sala = request.getParameterValues("salas");
            int nSala=0;
            for (int i = 0; i < sala.length; i++) {
                nSala=Integer.parseInt(sala[i]);
            }
            String[] desc = request.getParameterValues("descuento");
            HoraCharla hc = new HoraCharla(new Date(), Integer.parseInt(hora.substring(0,hora.indexOf(':'))),Integer.parseInt(hora.substring(hora.indexOf(':')+1))); 
            if(desc!=null){
                tarifa= tarifa*0.80;
            }
            Charla c = new Charla(hc, tema, lstSala.get(nSala).getId_sala(), (int)tarifa);
            String resultado = bd.grabarCharla(c);
            request.setAttribute("horaIndex", request.getParameter("index"));

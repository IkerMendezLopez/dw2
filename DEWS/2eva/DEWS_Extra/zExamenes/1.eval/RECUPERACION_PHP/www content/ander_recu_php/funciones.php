<?php
    ...
    function categoriasTemas($con){
        
        $categorias = array();
        
        $sql = "select distinct categoria from temas, cursos where cursos.idtema=temas.idtema";
    ...
    function temas($con, $catSeleccionada=""){
    ...
    function cantCursos($con, $idTema){
        
        $sql = "select count(*) as cantCursos from cursos where IDTEMA='$idTema'";
        $rs = mysqli_query($con, $sql);
        
        if($fila = mysqli_fetch_object($rs)){ 
            return $fila->cantCursos;
        }
    }
    
    function cursosCorrespondientes($con, $idTema){
    ...
    function actualizaPrecio($con, $idCurso, $precio){
    ...
    function cursosDeHoy($con, $fecha){
        
        $fecha;
        
        $sql = "select idcurso, asistentes from cursos where DIA='now()'";
